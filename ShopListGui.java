...
public  class ShopListGui
{
    ...
    public ShopListGui(ShopSearchGui parent, List<Shop>shops) {
        this.client = parent.getMinecraftInstance();
        this.listWidth = 160;
        this.listHeight = parent.height - 140;
        this.top = 100;
        this.bottom = parent.height - 40;
        this.slotHeight = 35;
        this.left = 40;
        this.right = 200;
    ...
    protected void elementClicked(int index, boolean var2) {
        this.parent.selectShopAtIndex(index);
    }

    protected boolean isSelected(int index){
    	return this.parent.shopIsSelected(index);
    }
    ...
    protected void drawSlot(int var1, int var2, int var3, int var4, Tessellator var5) {
    	Shop shop = shops.get(var1);
    	String distance = String.format("%.2f meters", shop.distanceFrom(this.parent.getMinecraftInstance().thePlayer));
    	String headline = String.format("%s - %s", shop.item, distance);

    	this.parent.getFontRenderer().drawString(this.parent.getFontRenderer().trimStringToWidth(headline, listWidth - 10), this.left + 3 , var3 + 2, 0xFFFFFF);
    	int line = 0;
    	if (shop.isBuying()) {
    		line++;
    		String buying = String.format("Buy %d for %d%s", shop.buyAmount, shop.buyPrice, shop.buyCurrency);
        	this.parent.getFontRenderer().drawString(this.parent.getFontRenderer().trimStringToWidth(buying, listWidth - 10), this.left + 3 , var3 + (line * 10) + 2, 0xFFFFFF);
    	}
    	if (shop.isSelling()) {
    		line++;
    		String selling = String.format("Sell %d for %d%s", shop.sellAmount, shop.sellPrice, shop.sellCurrency);
        	this.parent.getFontRenderer().drawString(this.parent.getFontRenderer().trimStringToWidth(selling, listWidth - 10), this.left + 3 , var3 + (line * 10) + 2, 0xFFFFFF);
    	}
    }
